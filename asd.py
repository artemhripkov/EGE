...
#25(маски)from fnmatch import *
#range меняется в зависимости от условия#шаг это делимостьfor i in range(0, 10**10, 1):
    if fnmatch(str(i), '12673788?*'):
        print(i)

#2from itertools import *
def f(x,y,w,z):
    return #в зависимости от условияfor a1,a2,a3,a4 in product([0,1], repeat = 4):
    #в зависимости от условия    t = [(1,1,1,a1),(1,1,1,a2),(1,a3,a4,1)]
    if len(t)==len(set(t)):
        for p in permutations('xywz'):
...
#19-21def f(a,b,m):
    if a+b>=231: return m%2==0    if m==0: return 0    h = [f(a+2,b,m-1),f(a*2,b,m-1),f(a,b+2,m-1),f(a,b*2,m-1)]
    return any(h) if (m-1)%2==0 else all|(h)
print('19)',[s for s in range(1,214) if f(17,s,2)]) #all->anyprint('20)',[s for s in range(1,214) if not f(17,s,1) and f(17,s,3)])
print('21)',[s for s in range(1,214) if not f(17,s,2) and f(17,s,4)])

#15(отрезки)from itertools import *

def f(x):
    b = 10<=x<=15    c = 18<=x<=40    a = a1<=x<=a2
    returnox = [i/4 for i in range(10*4, 40*4+1)]
m = []
...
#15(обычное)def f(x,y):
    return ((x-3*y)<a)or(y>400)or(x>56)

for a in range(0,1000):
    if all(f(x,y) for x in range(1,1000) for y in range(1,1000)):
        print(a)
        break#27f = open('7584a.txt')
f.readline()#если первая строчка x,ydata = [list(map(float, x.replace(',','.').split())) for x in a]
print(len(data))

def dist(p1,p2):
    x1,y1,x2,y2 = *p1, *p2
    return ((x1-x2)**2+(y1-y2)**2)**0.5def getCluster(p0):
